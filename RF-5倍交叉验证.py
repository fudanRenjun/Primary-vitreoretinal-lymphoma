...
# 加载CSV文件
data = pd.read_csv('E:/RS/LBL/Feature/RF/RF70-6.csv')
...
# 假设 y_test, y_pred_prob 已经有了（最后一个 fold 的结果）
...
plt.plot(thresholds, tpr, marker='s', linewidth=1,markersize=3, label='TPR')
plt.plot(thresholds, fpr, marker='^', linewidth=1,markersize=3, label='FPR')
plt.plot(thresholds, ks,  marker='D', linewidth=1,markersize=3, label='KS')

# 标记最优 KS 点和垂直阈值线
plt.vlines(best_thresh, 0, best_ks, linestyles='--', colors='grey')
plt.scatter([best_thresh], [best_ks], s=50, color='black')

# 添加文字注释
plt.text(best_thresh + 0.02, best_ks,
         f"(%.2f, %.2f)" % (best_thresh, best_ks),
         fontsize=6, va='bottom')

# 坐标轴和图例
plt.xlabel('Threshold',fontsize=8)
plt.ylabel('Metric Value',fontsize=8)
plt.title('TPR, FPR and KS vs. Threshold',fontsize=8)
plt.legend(loc='best', fontsize=5)
plt.xlim(0,1)
plt.ylim(0,1.05)
plt.grid(alpha=0.3)
plt.tick_params(axis='x', labelsize=6)  # x轴刻度字体大小
plt.tick_params(axis='y', labelsize=6)  # y轴刻度字体大小
plt.show()